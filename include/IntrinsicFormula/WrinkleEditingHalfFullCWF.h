...
{
	 // this is a variantion of the one we used to generate the paper results, where we manaully blend to get omega_tar, and ask the w-z consistency for (z, w_tar), and our variables are (z, w). We also added a term penalize the difference between w and w_tar
	class WrinkleEditingHalfFullCWF : public WrinkleEditingModel
	{
	public:
        WrinkleEditingHalfFullCWF(const Eigen::MatrixXd& pos, const MeshConnectivity& mesh, const std::vector<VertexOpInfo>& vertexOpts, const Eigen::VectorXi& faceFlag, int quadOrd, double spatialAmpRatio, double spatialEdgeRatio, double spatialKnoppelRatio, int effectivedistFactor) :
