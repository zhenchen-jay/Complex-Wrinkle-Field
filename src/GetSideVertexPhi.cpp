...
void getSideVertexPhi(const Eigen::MatrixXd& V, const MeshConnectivity& mesh, const Eigen::VectorXd& edgeOmega, const std::vector<std::complex<double>>& vertZvals, const std::vector<std::pair<int, Eigen::Vector3d>>& bary, Eigen::VectorXd& upPhi, int upLevel, int interpType)
{
    int nUpVerts = bary.size();
    upPhi.resize(nUpVerts);
...
void getSideVertexPhi(const Eigen::MatrixXd& V, const MeshConnectivity& mesh, const Eigen::VectorXd& edgeOmega, const std::vector<std::complex<double>>& vertZvals, Eigen::MatrixXd& upV, const MeshConnectivity& upMesh, Eigen::VectorXd& upPhi, int upLevel, int interpType)
{
    ...
    getSideVertexPhi(V, mesh, edgeOmega, vertZvals, bary, upPhi, upLevel, interpType);
}