                                             ...
                                             Eigen::Matrix<double, 6, 6> *hess)
{
    ...
    w2 = _inputFields.row(_mesh.faceVertex(f1, 0)) * x(3 * f1) + _inputFields.row(_mesh.faceVertex(f1, 1)) * x(3 * f1 + 1) + _inputFields.row(_mesh.faceVertex(f0, 2)) * x(3 * f1 + 2);
...
void PlaneWaveExtraction::testOptEnergyPerEdge(Eigen::VectorXd x, int eid)
{
    ...
    for(int i = 3; i < 10; i++)
    {
        ...
        for(int j = 0; j < 3; j++)
        {
            x(3 * f0 + j) = backupX(3 * f0 + j) + dir(j);
            x(3 * f1 + j) = backupX(3 * f1 + j) + dir(3 + j);
        }
