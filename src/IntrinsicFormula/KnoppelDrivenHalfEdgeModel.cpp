...
void KnoppelDrivenHalfEdgeModel::convertList2Variable(Eigen::VectorXd& x)
{
...
void KnoppelDrivenHalfEdgeModel::convertVariable2List(const Eigen::VectorXd& x)
{
    int nverts = _zvalsList[0].size();
    int nedges = _edgeOmegaList[0].rows();
...
double KnoppelDrivenHalfEdgeModel::computeEnergy(const Eigen::VectorXd& x, Eigen::VectorXd* deriv, Eigen::SparseMatrix<double>* hess, bool isProj)
{
    int nverts = _zvalsList[0].size();
    int nedges = _edgeOmegaList[0].rows();
    ...
    for(int i = 0; i < numFrames; i++) {
        ...
        double knoppel = IntrinsicFormula::KnoppelHalfEdgeEnergyGivenMag(_mesh, _refEdgeOmegaList[id],
...
void KnoppelDrivenHalfEdgeModel::testEnergy(Eigen::VectorXd x)
{
